...
// creating sieve till sq(upper limit)
const int N = 1e5;
...
void generatePrimesInRange(long long l, long long r){
	vector<int> primes;
    //storing all the primes till sqrt(r) will use these for creating segmented sieve.
	int lim = sqrt(r);
	for(int i = 2; i <= lim; ++i)
		if(sieve[i])
			primes.push_back(i);
    ...
    // marking all multiples of primes int the range of l to r.
	for(auto i:primes){
		long long mul = (l/i)*i;
